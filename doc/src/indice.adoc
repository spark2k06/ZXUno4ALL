...
Un agradecimiento especial a desUBIKado por la intensa y continua labor recopilando y recabando información sobre los distintos cores y funcionalidades.
...
Addon para ZXUnCore y ZXUnGo+ con conector de Joystick DB9 (Norma Atari), compatible con el https://www.zxuno.com/forum/viewtopic.php?f=25&t=3826[splitter de Antonio Villena].

[.text-center]
image:./img/ZXUnGoPlus_DB9.png[scaledwidth=90%]

Dispone de jumpers de configuración similares a los disponibles en un ZX-Uno original. Así, es posible configurar para que el `PIN5` sea `+5V` y el `PIN7` sea `FIRE3` (imagen de la izquierda a continuación), o bien que el `PIN5` sea `FIRE3` y el `PIN7` sea `+5V` (imagen de la derecha), lo que añade compatibilidad con el splitter de Antonio Villena si se desea.
...
La forma de utilizarlo consiste en conectarlo con ZXUnCore y, si se dispone de ZXUnGo+, conectarlo detrás. Una vez hecho esto, los cores con soporte de joystick podrán utilizar los joysticks que se conecten al puerto DB9 del addon.
...
- Una tarjeta SD, de 32GB de capacidad o menos
...
- Un ratón PS/2 (se necesita adaptador PS/2 en "Y")
...
- Comportamiento del joystick emulado con el teclado numérico de un teclado PS/2 (Joy Keypad) y del segundo joystick (con ZXUnGo+ DB9 y splitter): Kempston, Sinclair Joystick 1, Sinclair Joystick 2, Protek o Fuller
- Comportamiento de un joystick conectado a conectado al puerto principal de un ZXUnGo+ DB9 (Joy DB9): Kempston, Sinclair Joystick 1, Sinclair Joystick 2, Protek, Fuller o simular las teclas `Q`, `A`, `O`, `P`, `Espacio` y `M`
- Activar el segundo joystick (ZXUnGo+ DB9 y splitter). Sólo válido para ZXUnGo+ DB9 con un splitter conectado y para los cores que estén diseñados para soportarlo
...
El core principal es el que implementa un ordenador ZX Spectrum. Este core es especial, y no se puede sustituir por otro que no sea de ZX Spectrum, ya que el ZX-Uno lo utiliza para su funcionamiento. La versión instalada por defecto es una variación del core EXP27-160820 de mcleod_ideafix.
...
. Una vez el juego ha cargado, pulsando NMI (`F5`), aparecerá un campo en la parte superior izquierda de la pantalla
...
- `Ctrl+Alt+Bloq. Desp` (`Caps Shift+Symbol Shift+Z`, `Caps Shift+Symbol Shift+C` y luego `Caps Shift+Symbol Shift+Y` en ZXUnGo+): Cambio a Verde - Ambar - B&W (Simulación de monitor monocromático)
- `Ctrl+Alt+KeyPad -` (`Caps Shift+Symbol Shift+Z`, `Caps Shift+Symbol Shift+C` y luego `Caps Shift+Symbol Shift+R` en ZXUnGo+): Reducir velocidad
- `Ctrl+Alt+KeyPad +` (``Caps Shift+Symbol Shift+Z`, `Caps Shift+Symbol Shift+C` y luego `Caps Shift+Symbol Shift+D` en ZXUnGo+): Velocidad normal a 12MHz
- `Ctrl+Alt+Supr`(`Caps Shift+Symbol Shift+F` y luego `N` en ZXUnGo+): Soft Reset
- `Ctrl+Alt+Backspace` (`Caps Shift+Symbol Shift+F` y luego `B` en ZXUnGo+): Hard Reset. Backspace es la tecla de borrar hacia atrás, encima de `Enter`
...
- `F1` (`Caps Shift+Symbol Shift+1` en ZXUnGo+): Habilitar o deshabilitar los caracteres alternativos
- `F5` (`Caps Shift+Symbol Shift+5` en ZXUnGo+) o el botón 2 del joystick para mostrar u ocultar el menú.
- `F9` (`Caps Shift+Symbol Shift+9` en ZXUnGo+): Deshabilita o habilita la salida MIC al conector de audio, porque algunos juegos generan un ruido molesto
- `F10` (`Caps Shift+Symbol Shift+0` en ZXUnGo+): Habilita o deshabilita sonido de entrada por la salida de audio, para escuchar la cinta si se carga por audio
- `Bloq. Despl.` (`Caps Shift+Symbol Shift+Y` en ZXUnGo+: para cambiar entre modo de vídeo RGB y VGA
- `Ctrl+Alt+Supr` (`Caps Shift+Symbol Shift+F` y luego `B` en ZXUnGo+): Reset
- `Ctrl+Alt+Backspace` (`Caps Shift+Symbol Shift+F` y luego `B` en ZXUnGo+): Hard reset.

==== Guía básica

Pulsando `F5` (`Caps Shift+Symbol Shift+5` en ZXUnGo+) o el botón 2 del joystick se muestra el menú de configuración. Se usan las teclas de cursor (`Caps Shift+5`, `Caps Shift+6`, `Caps Shift+7` y `Caps Shift+8` en ZXUnGo+) y `Enter` para elegir y seleccionar opciones del menú.
...
Existen también opciones para ajustar el tiempo de espera inicial de la BIOS, el core a ejecutar por defecto, o la ROM de Spectrum a utilizar por defecto. Véase la https://kounch.github.io/zx123_tool/es/manual.html[documentación de la herramienta] para más información.
...
Véase la https://kounch.github.io/zx123_tool/es/manual.html[documentación de la herramienta] para ver todas las posibles opciones a la hora de añadir una ROM de Spectrum.
...
==== Recuperación vía audio

Si al arrancar el ZX-Uno con un joystick conectado y manteniendo la palanca hacia arriba y el botón de disparo pulsado, sale algo parecido a una carta de ajuste, entonces el ZX-Uno está esperando carga de BIOS usando la entrada de sonido.

*Material necesario*:

- Un ZX-Uno con conector de joystick

- Un joystick compatible

- Cable de audio y algún mecanismo de <<#_carga_desde_cinta,carga desde cinta>> que pueda interpretar archivos `.TAP`

- Tarjeta SD

*Software necesario*:

- Un fichero `firmware.tap` (por ejemplo, el que está disponible http://desubikado.sytes.net/wp-content/uploads/firmware.tap_.zip[aquí]) 

- Un fichero `FIRMWARE.ZX1` (recordar que la última versión de los ficheros de firmware se puede descargar desde https://github.com/zxdos/zxuno/tree/master/firmware[el repositorio oficial]), que se ha de copiar en la raíz de la tarjeta SD

[NOTE]
====
El fichero `firmware.tap` se puede crear con el comando
...
La utilidad GenRom se puede obtener desde https://github.com/zxdos/zxuno/tree/master/sdk/src/tools[el repositorio oficial] (versión compilada para Windows, https://github.com/zxdos/zxuno/tree/master/sdk/windows-x86/bin[disponible aquí]).
====

*Pasos a seguir*:

. Insertar la tarjeta SD y arrancar el ZX-Uno con un joystick conectado y manteniendo la palanca hacia arriba y el botón de disparo pulsado

. Reproducir el archivo `firmware.tap`

. Se ha de estar muy atento a la finalización de la carga, para pulsar rápidamente `F2` (`Caps Shift+1` en ZXUnGo+) y así entrar en la BIOS (si esto fallase, habría que volver a comenzar todo el proceso desde el principio)

. Una vez dentro de la BIOS, seleccionar `Upgrade`, elegir __"Upgrade BIOS for ZX"__, y luego __"SDfile"__. El sistema leerá el fichero `FIRMWARE.ZX1` y avisará cuando esté actualizado.

Si el proceso finaliza correctamente, reiniciar el ZX-Uno y será posible usar cualquiera de los métodos normales para <<#_actualizaciones,actualizar ROMs, Cores, la memoria flash, etc.>>

==== Preparación del cableado especial para recuperación

En los siguientes pasos de recuperación se habla de conectar cables puente o USB-Blaster a la placa ZXUnCore. Para ello, usar la siguiente imagen como referencia.
...
[start=3]
. Conectar el USB-Blaster o los cables puente para GPIO <<#_preparación_del_cableado_especial_para_recuperación,tal y como se explica anteriormente>>. Si se va a hacer la conexión usando GPIO, conectar las líneas de JTAG de la FPGA (`TMS`, `TDI`, `TDO`, `TCK` y `GND`), con los cables, a los pines https://es.wikipedia.org/wiki/GPIO[GPIO] de la Raspberry Pi.
...
- Cable USB-Blaster <<#_preparación_del_cableado_especial_para_recuperación,preparado con las conexiones adecuadas para ZXUnCore>>
- Imagen Flash y recovery para ZX-Uno, del https://github.com/zxdos/zxuno/tree/master/modflash[repositorio de Github] y el https://www.zxuno.com/forum/viewtopic.php?f=37&t=774[foro de ZX-Uno]
...
[NOTE]
====
En Linux, usando el mismo software UrJTAG, el proceso debería ser similar, aunque en vez de instalar las dependencias (libftdi libusb pkg-config) mediante brew, se debería hacer mediante apt, yum o similar
====
...
https://zxuno.com/forum/viewtopic.php?f=27&t=1288[ROM Secreta en ZX-Uno]