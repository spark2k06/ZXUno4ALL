...
[align="center",width="100%",%header,cols=6*,options="header"]
|===
|Button
|Kempston
|Sinclair 1
...
[WARNING]
====
Some versions of this core do not initialize properly the keyboard when using only a membrane keybard. You have to cold boot the ZXUnGo+ with a PS/2 keyboard attached.
====
...
The ZX-Uno core has been made by Ques and enhanced after by azesmbog and hoglet.
...
[start=4]
. Copy `FLASH.ZX1` file to the root of the ZX-Uno microSD card.
...
[start=10]
. Insert in the ZX-Uno the microSD card formatted as FAT16 o FAT32, and with the `FLASH.ZX1` file https://github.com/zxdos/zxuno/raw/master/modflash/FLASH.ZX1[downloaded previously].
...
. Save changes:
- `Exit` -> `Save changes and exit`
. Completely turn off the ZXUnGo+ and turn it on again
====
...
- USB-Blaster cable. Using the right pins for ZXUnCore, <<#_jtag_cable_connections,as explained earlier>>
- Flash Image file and and recovery file for ZX-Uno. The same ones as for Raspberry Pi, from the https://github.com/zxdos/zxuno/tree/master/modflash[official repository]
...
[start=10]
. Insert in the ZX-Uno the microSD card formatted as FAT16 or FAT32, and with the `FLASH.ZX1` file https://github.com/zxdos/zxuno/raw/master/modflash/FLASH.ZX1[downloaded previously].
